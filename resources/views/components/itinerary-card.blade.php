...
<div class="bg-gradient-to-br from-white via-outdoor-cream-50 to-white rounded-3xl shadow-2xl border-2 border-outdoor-forest-200/30 overflow-hidden hover:shadow-outdoor-brutal transition-all duration-500 group hover:-translate-y-2 hover:rotate-1 backdrop-blur-sm relative">
    ...
    {{-- Image de l'itinéraire avec overlay dramatique --}}
    <div class="relative aspect-w-16 aspect-h-9 bg-gradient-to-br from-outdoor-forest-400 to-outdoor-forest-700 overflow-hidden">
        ...
        @if($itinerary->featuredImage && $itinerary->featuredImage->image_path)
            <img src="{{ $itinerary->featuredImage->medium_image }}" 
                 alt="{{ $itinerary->title }}"
        ...
        @else
            <div class="w-full h-56 bg-gradient-to-br from-outdoor-olive-500 via-outdoor-forest-600 to-outdoor-earth-700 flex items-center justify-center relative z-5">
                <div class="text-center text-white relative z-10">
                    ...
                    <div class="text-lg font-bold tracking-wider">AVENTURE OUTDOOR</div>
        ...
        @if($itinerary->published_at)
            <div class="absolute bottom-4 right-4 z-20">
                <div class="bg-outdoor-forest-800/95 text-outdoor-cream-100 text-xs px-3 py-1.5 rounded-xl backdrop-blur-md border border-outdoor-forest-600/50 shadow-xl font-bold">
                    {{ $itinerary->published_at->format('d/m/Y') }}
                </div>
            </div>
        @endif
    </div>
    ...
    <div class="p-8 relative z-10">
        ...
        <div class="mb-6">
            ...
            <p class="text-outdoor-forest-600 leading-relaxed line-clamp-3 font-medium">
                {{ Str::limit($itinerary->description, 130) }}
            </p>
        ...
        @if($itinerary->distance_km || $itinerary->elevation_gain_m || $itinerary->estimated_duration_minutes)
                ...
                <div class="group/stat relative">
                    ...
                    <div class="relative bg-gradient-to-br from-outdoor-olive-500 to-outdoor-olive-600 rounded-2xl p-4 text-center shadow-xl border border-outdoor-olive-400 transform group-hover/stat:scale-105 transition-all duration-300">
                        <div class="text-2xl font-black tracking-tighter text-outdoor-cream-50">
                            {{ number_format($itinerary->distance_km, 1) }}
                        </div>
            ...
            @if($itinerary->estimated_duration_minutes)
                <div class="group/stat relative">
                    ...
                    <div class="relative bg-gradient-to-br from-outdoor-ochre-500 to-outdoor-ochre-600 rounded-2xl p-4 text-center shadow-xl border border-outdoor-ochre-400 transform group-hover/stat:scale-105 transition-all duration-300">
                        <div class="text-2xl font-black tracking-tighter text-outdoor-cream-50">
                            {{ floor($itinerary->estimated_duration_minutes / 60) }}h{{ $itinerary->estimated_duration_minutes % 60 > 0 ? sprintf('%02d', $itinerary->estimated_duration_minutes % 60) : '' }}
                        </div>
        ...
        <div class="flex items-center justify-between mb-6 px-2">
            @if($itinerary->user)
                <div class="flex items-center bg-outdoor-cream-100 px-3 py-2 rounded-xl border border-outdoor-cream-200">
                    ...
                    <span class="text-sm font-bold text-outdoor-forest-700">{{ $itinerary->user->name }}</span>
                </div>
            @endif
            ...
            @if($itinerary->images->count() > 0)
                <div class="flex items-center bg-outdoor-ochre-100 px-3 py-2 rounded-xl border border-outdoor-ochre-200">
                    ...
                    <span class="text-sm font-bold text-outdoor-forest-700">{{ $itinerary->images->count() }} photo{{ $itinerary->images->count() > 1 ? 's' : '' }}</span>
                </div>
            @endif
        </div>
        ...
        <div class="flex gap-4">
            ...
            @if($itinerary->gpx_file_path)
                <div class="relative group/gpx">
                    ...
                    <a href="{{ asset('storage/' . $itinerary->gpx_file_path) }}" 
                       download
